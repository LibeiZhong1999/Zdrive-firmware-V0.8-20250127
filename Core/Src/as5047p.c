...
AS5047P_Result tx_buffer[3],rx_buffer[3];
...
/************************************************************************************
 *
 * 	Function: 	AS5047P_GetAngle_Dir(instance)
 * 	Access:		public
 * 	Inputs:
 * 		instance:	Pointer to the encoder's instance.
 *
 ...
 *
 * 	Description:
 *	Function reads current encoder's position from AS5047P_ANGLECOM register directly using SPI DMA.
 *
 ************************************************************************************/
void Load_tx_buffer()
{
	AS5047P_Result frame;
	
	frame = AS5047P_ANGLECOM & AS5047P_FRAME_DATA;
  frame |= AS5047P_ACCESS_READ << 14;
  frame |= AS5047P_CalcParity(frame) << 15;
	tx_buffer[0]=frame;
	
	frame = AS5047P_NOP & AS5047P_FRAME_DATA;
  frame |= AS5047P_ACCESS_READ << 14;
  frame |= AS5047P_CalcParity(frame) << 15;
	tx_buffer[1]=frame;
...
//	spiStatus = HAL_SPI_TransmitReceive_DMA(&hspi3, (uint8_t *)tx_buffer, (uint8_t *)rx_buffer, 2);
	
	HAL_GPIO_WritePin(SPI_CS_GPIO_Port, SPI_CS_Pin, 0x00);
	spiStatus = HAL_SPI_TransmitReceive(&hspi3, (uint8_t *)tx_buffer, (uint8_t *)rx_buffer, 2,1000);
	HAL_GPIO_WritePin(SPI_CS_GPIO_Port, SPI_CS_Pin, 0x01);
	
//	HAL_GPIO_WritePin(SPI_CS_GPIO_Port, SPI_CS_Pin, 0x00);
//	spiStatus = HAL_SPI_TransmitReceive(&hspi3, (uint8_t *)tx_buffer, (uint8_t *)rx_buffer, 1,1000);
//	HAL_GPIO_WritePin(SPI_CS_GPIO_Port, SPI_CS_Pin, 0x01);
//	for(int i=0;i<100000000;++i);
//	HAL_GPIO_WritePin(SPI_CS_GPIO_Port, SPI_CS_Pin, 0x00);
//	spiStatus = HAL_SPI_TransmitReceive(&hspi3, (uint8_t *)(tx_buffer+1), (uint8_t *)(rx_buffer+1), 1,1000);
//	HAL_GPIO_WritePin(SPI_CS_GPIO_Port, SPI_CS_Pin, 0x01);
	...
	if( spiStatus != HAL_OK)
  {
      return -1;
			...
			return (rx_buffer[1]&AS5047P_FRAME_DATA);
}
